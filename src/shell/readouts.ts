...
export interface ReadoutDef { key: string; symbol: string; label: Text; unit: string; transform?: (v: number) => number; hint?: Text; /** 只在某些情況顯示（如第二顆球存在時）；不影響物理 */ visible?: (obs: Record<string, number>) => boolean }
...
export function formatReadouts(obs: Record<string, number>, defs: ReadoutDef[]): { key: string; symbol: string; label: Text; text: string; hint?: Text }[] {
  return defs.filter(d => d.visible?.(obs) ?? true).map(d => {
    const raw = obs[d.key];
