...
export function formatReadouts(obs: Record<string, number>, defs: ReadoutDef[]): { key: string; symbol: string; label: Text; text: string }[] {
  return defs.map(d => {
    const raw = obs[d.key];
    const v = raw === undefined ? NaN : (d.transform ? d.transform(raw) : raw);
    return { key: d.key, symbol: d.symbol, label: d.label, text: withUnit(v, d.unit || undefined) };
  });
}
