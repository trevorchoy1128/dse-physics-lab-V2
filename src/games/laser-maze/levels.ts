...
export const LEVELS: Level[] = [
  {
    id: "mirror", name: { zh: "熱身：一面鏡", en: "Warm-up: one mirror" },
    brief: { zh: "激光向右射。把平面鏡放在光路上，轉到合適角度，令光線轉向下面的探測器。", en: "The laser fires to the right. Put the plane mirror in the beam and turn it so the ray reaches the detector below." },
    ...
    lasers: [{ pos: [0.6, 4.5], dir: 0 }], glass: [], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [5.5, 0.8], r: 0.3 },
    slots: [[5.5, 4.5], [8.5, 4.5]], inventory: { mirror: 1 },
  },
  {
    id: "corner", name: { zh: "繞過牆", en: "Round the wall" },
    ...
    lasers: [{ pos: [0.6, 6.5], dir: 0 }], glass: [], mirrors: [], lenses: [], blocks: [wall(5.6, 6.4, 0, 5)],
    target: { kind: "spot", c: [11.3, 2.5], r: 0.3 },
    slots: [[3.5, 6.5], [8.5, 6.5], [8.5, 2.5], [3.5, 2.5]], inventory: { mirror: 2 },
  },
  {
    id: "angle", name: { zh: "反射角", en: "Angle of reflection" },
  ...
  },
  {
    ...
    glass: [{ n: N_WATER, edges: rectEdges(0, 0, 12, 5), name: { zh: "水", en: "water" }, label: { zh: "水 n = 1.33", en: "water n = 1.33" }, labelAt: [0.3, 4.6] }], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [7.5, 0.5], r: 0.2, fish: true },
    slots: [], inventory: {}, aimLine: true,
  },
  {
    id: "semicircle", name: { zh: "半圓玻璃塊：折射出去", en: "Semicircular block: refract out" },
    ...
    glass: [{ n: N_GLASS, edges: semicircleEdges([6, 4], 2.5, 90), name: { zh: "玻璃", en: "glass" }, label: { zh: "玻璃 n = 1.5", en: "glass n = 1.5" }, labelAt: [5.1, 2.2] }], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [6 + 3 * Math.sin(Math.PI / 4), 4 + 3 * Math.cos(Math.PI / 4)], r: 0.2 },
    slots: [], inventory: {},
  },
  {
    id: "tir", name: { zh: "全內反射", en: "Total internal reflection" },
  ...
  {
    id: "prism", name: { zh: "直角稜鏡轉 90°", en: "Right-angle prism: turn 90°" },
    brief: { zh: "沒有鏡，只有一個直角稜鏡。放好、轉好，令光垂直射入一面，在斜面全內反射，再垂直射出。", en: "No mirror this time, just a right-angle prism. Place and turn it so the ray enters one face at right angles, totally reflects at the hypotenuse and leaves at right angles." },
    ...
    lasers: [{ pos: [0.6, 5.5], dir: 0 }], glass: [], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [6.5, 1.2], r: 0.3 },
    slots: [[6.5, 5.5], [9.5, 5.5]], inventory: { prism: 1 },
  },
  {
    id: "porro", name: { zh: "稜鏡回頭 180°", en: "Prism: turn back 180°" },
    ...
    note: { zh: "稜鏡 n = 1.5，臨界角 41.8°。", en: "Prism n = 1.5, C = 41.8°." },
    lasers: [{ pos: [1.6, 4.75], dir: 0 }], glass: [], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [3.2, 4.25], r: 0.15 },
    slots: [[8.5, 4.5], [8.5, 2.5]], inventory: { prism: 1 },
  },
  {
    id: "fibre", name: { zh: "光纖不可漏光", en: "Keep the light in the fibre" },
    ...
    glass: [{ n: N_WATER, edges: rectEdges(3, 3.5, 10.5, 4.5), name: { zh: "光纖", en: "fibre" }, label: { zh: "光纖 n = 1.33", en: "fibre n = 1.33" }, labelAt: [6, 5] }], mirrors: [], lenses: [], blocks: [],
    target: { kind: "strip", a: [10.62, 3.15], b: [10.62, 4.85] },
    slots: [], inventory: {}, stars: { kind: "band", three: 59, two: 45 },
  },
  {
    id: "focus", name: { zh: "平行光聚焦", en: "Focus parallel rays" },
    brief: { zh: "三束平行激光。用一個凸透鏡把三束光全部聚到探測器上。透鏡放在哪一格？", en: "Three parallel beams. Use one convex lens to bring all three onto the detector. Which slot?" },
    hint: { zh: "平行主軸的光經凸透鏡後會聚於焦點。本關 f = 2 格，所以透鏡要放在探測器前 2 格，並垂直於光線。", en: "Rays parallel to the principal axis converge at the focus. Here f = 2 squares, so the lens goes 2 squares in front of the detector, perpendicular to the beams." },
    note: { zh: "凸透鏡焦距 f = 2 格，當薄透鏡。", en: "Convex lens, f = 2 squares, treated as a thin lens." },
    lasers: [{ pos: [0.6, 5.5], dir: 0 }, { pos: [0.6, 4.5], dir: 0 }, { pos: [0.6, 3.5], dir: 0 }], glass: [], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [9.5, 4.5], r: 0.15 },
    slots: [[5.5, 4.5], [6.5, 4.5], [7.5, 4.5], [8.5, 4.5]], inventory: { lens: 1 }, lensF: 2,
  },
  {
    id: "lens-formula", name: { zh: "透鏡公式", en: "The lens formula" },
    brief: { zh: "點源與探測器相距 5 格，透鏡 f = 1.2 格。放在哪裏才會聚到探測器？有兩個位置，都找到才有三星。", en: "Point source and detector are 5 squares apart; the lens has f = 1.2 squares. Where does it form the image on the detector? Two positions work: find both for three stars." },
    hint: { zh: "1/u + 1/v = 1/f，u + v = 5，f = 1.2：uv = 6，所以 u = 2 或 3。", en: "1/u + 1/v = 1/f with u + v = 5 and f = 1.2 gives uv = 6, so u = 2 or 3." },
    note: { zh: "凸透鏡焦距 f = 1.2 格，當薄透鏡；點源發出三條光線。", en: "Convex lens, f = 1.2 squares, thin lens; the point source emits three rays." },
    lasers: [{ pos: [1.5, 4.5], dir: 14 }, { pos: [1.5, 4.5], dir: 0 }, { pos: [1.5, 4.5], dir: -14 }], glass: [], mirrors: [], lenses: [], blocks: [],
    target: { kind: "spot", c: [6.5, 4.5], r: 0.15 },
    slots: [[2.5, 4.5], [3.5, 4.5], [4.5, 4.5], [5.5, 4.5]], inventory: { lens: 1 }, lensF: 1.2, stars: { kind: "twin" },
  },
];
