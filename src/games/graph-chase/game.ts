...
export const MAX_CATCHUP = 1.5;      // 一幀最多補算這麼多秒；相隔更久（分頁曾被隱藏）視作暫停，只走一步

/** 下一個轉折點時刻（不含 t = 0 與終點）；沒有則 null。輔助模式在轉折點自動暫停用。 */
export function nextCorner(L: Level, t: number): number | null {
  for (let i = 1; i < L.points.length - 1; i++) if (L.points[i][0] > t + 1e-9) return L.points[i][0];
  return null;
}

/**
 * 追上真實時間：把 run 走到 wallT（自開始起的遊戲秒數）。stopAt 給定時最多走到該時刻（轉折點自動暫停）。
 * 回傳 done（到終點）、stopped（停在 stopAt）、skipped（因暫停而略過的秒數，畫面把時鐘起點順延這麼多）。
 */
export function advance(L: Level, run: Run, wallT: number, a: number, stopAt?: number): { done: boolean; stopped: boolean; skipped: number } {
  let skipped = 0;
  if (wallT - run.t > MAX_CATCHUP) { skipped = wallT - run.t - DT; wallT = run.t + DT; }
  if (stopAt !== undefined) wallT = Math.min(wallT, stopAt);
  let done = run.t >= durationOf(L) - 1e-9;
  while (run.t < wallT - 1e-9 && !done) done = step(L, run, a);
  const stopped = !done && stopAt !== undefined && run.t >= stopAt - 1e-9;
  return { done, stopped, skipped };
}
