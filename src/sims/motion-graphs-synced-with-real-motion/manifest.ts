...
export const manifest: SimManifest = {
  ...
  beyondSpec: [
    "規格無參數表：初速 u 範圍 −5 至 5 m s⁻¹、加速度 a 範圍 −10 至 10 m s⁻²、時間窗 5 / 10 / 20 s，由開發端定，待老師定奪",
    "「由圖生成運動」以每秒一個可拖動的控制點畫 v–t 折線（共 T + 1 點），而非自由手繪，待老師定奪",
  ],
  version: "0.4.2",   // 0.4.2：軌道改為固定比例＋鏡頭跟車（背景流動、距離柱），線圖 t 軸刻度移到 y = 0 軸線上，線圖軸範圍由參數預測一次定好、不隨時間變（老師要求）；0.4.1：meta 用歸零值、拖動改為按時間就近、還原預設不改播放狀態； 0.4.0：折線末節點 a 不再 NaN、凍結 t 截斷為 T、畫面用歸零後的 s；0.2.0：時間窗末端 a 不再歸零（第 1 輪 F1）；0.3.0：observe 把 < 1 nm 的位移殘餘歸零（第 3 輪 F6）
};
