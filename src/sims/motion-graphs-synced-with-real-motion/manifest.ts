...
export const manifest: SimManifest = {
  ...
  beyondSpec: [
    ...
    "「由圖生成運動」以 11 個可拖動的控制點畫 v–t 折線（T ≤ 10 s 時每秒一點，T > 10 s 時平均分佈於 0 至 T），而非自由手繪，待老師定奪",
  ],
  version: "0.6.2",   // 0.6.2：線圖軸範圍去掉 1.02 邊際，曲線到達預測上限時軸不再跳級（老師 2026-09-11）；軸政策搬到 plan.ts axesFor 並加每幀相同的測試；0.6.1：跨節點的步分兩步走（梯形精確）、節點時間貼齊 1e-9（第二實作者第 9 輪）；掃掠式拖動、v 圖刻度落在圓點時間、t = 0 時還原預設也重算軸（學生試用者第 9 輪）；0.6.0：畫圖模式控制點間距 nodeDt = max(1, T/10)，令折線橫跨整個時間窗（第 8 輪：T = 45 s 時 11 點擠在 0–10 s）；圓點底部秒數改由 y = 0 軸刻度承擔；0.5.1：畫圖模式 s 軸範圍改為逐段精確極值並計入控制點以外的保持段（核數員第 7 輪 F9）；0.5.0：時間窗改為 2–60 s 滑桿（控制範圍改動），畫圖模式控制點按密度縮小、秒數標籤按間距疏化；0.4.2：軌道改為固定比例＋鏡頭跟車（背景流動、距離柱），線圖 t 軸刻度移到 y = 0 軸線上，線圖軸範圍由參數預測一次定好、不隨時間變（老師要求）；0.4.1：meta 用歸零值、拖動改為按時間就近、還原預設不改播放狀態； 0.4.0：折線末節點 a 不再 NaN、凍結 t 截斷為 T、畫面用歸零後的 s；0.2.0：時間窗末端 a 不再歸零（第 1 輪 F1）；0.3.0：observe 把 < 1 nm 的位移殘餘歸零（第 3 輪 F6）
};
