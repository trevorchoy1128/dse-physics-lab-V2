...
describe("自由下落（範本）畫面", () => {
  ...
  it("所有箭嘴由物體位置出發；重量箭嘴 = m g 向下", () => {
    let s = model.init(p); s = model.step(s, p, 0.5);
    const pl = plan(s, p, model.observe(s, p), all);
    for (const a of pl.arrows) expect(a.origin).toEqual([0, s.y, 0]);
    expect(pl.arrows.find(x => x.kind === "weight")!.vector).toEqual([0, -p.m * p.g, 0]);
  });
  it("同一 kind 一個縮放係數；標籤 = observe；單位在允許清單內", () => {
    const s = model.init(p); const obs = model.observe(s, p); const pl = plan(s, p, obs, all);
  ...
  it("圖層關閉時不畫該箭嘴", () => {
    ...
    expect(plan(s, p, model.observe(s, p), { velocity: false, acceleration: false, weight: false }).arrows).toHaveLength(0);
  });
  it("控制範圍內隨機 50 組參數：無 NaN / Infinity，箭嘴長度有界", () => {
    let seed = 1; const rnd = () => (seed = (seed * 16807) % 2147483647) / 2147483647;
    for (let k = 0; k < 50; k++) {
      const q: P = { h: 1 + rnd() * 49, m: 0.1 + rnd() * 4.9, g: [9.81, 9.8, 10][k % 3] };
      let s = model.init(q);
      for (let i = 0; i < 300; i++) {
        s = model.step(s, q, 1e-2);
        for (const a of plan(s, q, model.observe(s, q), all).arrows) for (const c of a.vector) { expect(Number.isFinite(c)).toBe(true); expect(Math.abs(c)).toBeLessThan(1e4); }
      }
    }
  });
});
