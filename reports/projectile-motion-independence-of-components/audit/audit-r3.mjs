// 物理核數 第 3 輪（v0.2.1；plan 改標籤位置、drop 時無 ycol）：拋體運動（Book 2 模擬器 2）獨立計算與逐幀比對
// 只用規格方程與自己的 RK4；不讀 model.ts / plan.ts。
// 用法：node audit-r2.mjs  →  印出摘要並寫 results-r3.json
...
const EXTRA = join(HERE, "extra-data-r3");
const K_AIR = 0.01, M = 1, DEG = Math.PI / 180;
const TOL_AN = 1e-6, TOL_NUM = 1e-4, TOL_VEC = 1e-9;
const Z_ACC = 0.8;            // 本輪：加速度箭嘴起點 z 偏移 0.8
const LABEL_Y_X = -3.6;       // y 標籤 [−3.6, y·k + r, 0]
const LABEL_X_Y = 0.05, LABEL_X_Z = 1.4; // 本輪：x 標籤 [x·k, 0.05, 1.4]
...
function auditRun(run, source) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (lx) { vmax("labX", Math.abs(lx.value - o.x)); vmax("labXpos", d3(lx.position, [o.x * k, LABEL_X_Y, LABEL_X_Z])); } else issue(`frame ${i}: 無 x 標籤`);
    for (const l of f.labels) if (l.position[1] < 0) issue(`frame ${i}: 標籤 ${l.symbol} y=${l.position[1]} < 0`);
    ...
    if (hasBodies) {
      ...
      for (let j = 0; j < cntA; j++) { const idx = Math.round(j * STROBE / dt); const e = A.frames[Math.min(idx, N - 1)]; const P = [e.x * k, e.y * k + r];
        ...
        if (f.meta.dropB) { if (yc) issue(`frame ${i}: drop 仍有 ycol-a-${j}`); } else if (yc) { bmax("ycolA", d3(yc.position, [YCOL_X, P[1], sideZ])); if (yc.shape !== "cylinder") issue(`frame ${i}: ycol-a 形狀`); } else issue(`frame ${i}: 無 ycol-a-${j}`); }
...
writeFileSync(join(HERE, "results-r3.json"), JSON.stringify({ results, consistency }, null, 1));
