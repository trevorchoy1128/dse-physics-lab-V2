// 第 3 輪補充核數（重跑）：對稱（內插）、射程、兩球同高、最高點讀數、負角 H、阻力落地時刻、done/時鐘、頻閃間距、縮放規則
...
{ const d = load("extra-data-r3", "official-scenario-range-45"); const f = d.frames[0]; const k = f.meta.k; for (const th of [15, 30, 45, 60, 75]) { const g = f.trails.find((q) => q.key === `ghost-${th}`); console.log(`ghost-${th}: 末點 x/k=${g.last[0] / k} 規格 R=${225 * Math.sin(2 * th * DEG) / 9.81} y=${g.last[1]} (r=${f.meta.r}) 首點=${JSON.stringify(g.first)} n=${g.n}`); } }
console.log("\n== 現象 1：兩球同高（y 對 yB）與同時落地 ==");
for (const [dir, n] of [["../data", "scenario-same-landing"], ["../data", "default"], ["extra-data-r3", "drop-default"], ["extra-data-r3", "air-default-fast"]]) {
...
console.log("\n== 負投射角的 H 讀數 vs 規格公式 ==");
for (const [dir, n] of [["../data", "random-3"], ["extra-data-r3", "edge-theta-30-h0"], ["extra-data-r3", "edge-u50-theta-30-h50-air"]]) {
...
for (const [dir, n] of [["../data", "random-0"], ["../data", "random-1"], ["../data", "random-2"], ["extra-data-r3", "edge-u50-theta45-h50-g10-air"], ["extra-data-r3", "edge-u50-theta-30-h50-air"], ["extra-data-r3", "air-default-fast"], ["extra-data-r3", "air-u50-theta40-h0-drop"]]) {
...
for (const n of ["official-default", "official-scenario-same-landing", "official-random-0", "edge-u50-theta-30-h50-air", "air-u50-theta40-h0-drop", "edge-u50-theta45-h50-g1.6", "edge-theta-30-h0"]) {
  const d = load("extra-data-r3", n), fr = d.frames; const i = d.doneAt; const f = i !== null ? fr[i] : null; const prev = i ? fr[i - 1] : null;
