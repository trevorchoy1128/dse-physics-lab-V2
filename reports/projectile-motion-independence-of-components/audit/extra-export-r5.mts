// 第 5 輪核數用（v0.3.1；同第 4 輪清單另加 5 個運行：F = −k v、質量 m、預設無第二顆球、photo 圖層）：以作者的 model/plan 產生額外運行，
...
const runs: { name: string; params: any; N?: number; layers?: any }[] = [
  ...
  { name: "air-u50-theta-30-h50-m5-drop", params: { ...defaults, u: 50, theta: -30, h: 50, m: 5, air: true, companion: "drop" }, N: 8000 },
  { name: "air-u50-theta45-h50-m0.1-drop", params: { ...defaults, u: 50, theta: 45, h: 50, m: 0.1, air: true, companion: "drop" }, N: 24000 },
  { name: "air-u50-theta45-h0-m0.1", params: { ...defaults, u: 50, theta: 45, h: 0, m: 0.1, air: true, companion: "none" }, N: 12000 },
  { name: "air-u1-theta-30-h0.5-m0.1", params: { ...defaults, u: 1, theta: -30, h: 0.5, m: 0.1, air: true, companion: "drop" }, N: 4000 },
  { name: "air-u50-theta90-h50-m5-g1.6", params: { ...defaults, u: 50, theta: 90, h: 50, m: 5, g: 1.6, air: true, companion: "none" }, N: 40000 },
  { name: "default-nophoto", params: { ...defaults }, layers: noPhoto },
];
const outDir = "C:/Users/trevor/dev/dse-physics-lab/reports/projectile-motion-independence-of-components/audit/extra-data-r5";
mkdirSync(outDir, { recursive: true });
const index: any[] = [];
for (const r of runs) {
  if (existsSync(`${outDir}/${r.name}.json`)) { index.push({ name: r.name, params: r.params, frames: r.N ?? 4000 }); console.error("skip (exists)", r.name); continue; }
