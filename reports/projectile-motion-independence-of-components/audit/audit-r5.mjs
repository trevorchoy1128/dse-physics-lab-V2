// 物理核數 第 5 輪（v0.3.1：model 未改；meta.tf 改為落地時刻 + 0.3 s，含線性阻力閉式解；有阻力時不畫 ½mvₓ² 線）
// 與第 4 輪差別：EXTRA 指向 extra-data-r5；meta.tf 預期 = max(t_f,A, t_f,B) + 0.3（阻力用我的解析落地時刻）；另與 extra-data-r4 逐位比對 model 輸出
// 只用規格方程 + 線性阻力解析解 + 自寫 RK4；不讀 model.ts / plan.ts。用法：node --max-old-space-size=12000 audit-r4.mjs
import { readFileSync, writeFileSync, existsSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
const HERE = dirname(fileURLToPath(import.meta.url));
const DATA = join(HERE, "..", "data"), EXTRA = join(HERE, "extra-data-r5");
...
function auditRun(run, source) {
  ...
  const metaTfExp = tfAll + 0.3; const tfMargin = mt.tf - tfAll;
...
const results = []; const consistency = []; const R4 = join(HERE, "extra-data-r4");
const cmpRuns = (a, b, full) => { let diff = 0, first = null, metaKeys = new Set(), obsDiff = 0, arrDiff = 0; const n = Math.min(a.frames.length, b.frames.length); for (let i = 0; i < n; i++) { const x = a.frames[i], y = b.frames[i]; if (JSON.stringify(x.obs) !== JSON.stringify(y.obs)) obsDiff++; if (JSON.stringify([x.arrows, x.labels, x.scales]) !== JSON.stringify([y.arrows, y.labels, y.scales])) arrDiff++; const mx = { ...x.meta }, my = { ...y.meta }; if (!full) { delete mx.tf; delete my.tf; } for (const k of new Set([...Object.keys(x.meta), ...Object.keys(y.meta)])) if (x.meta[k] !== y.meta[k]) metaKeys.add(k); if (JSON.stringify([x.obs, x.arrows, x.labels, x.scales, mx]) !== JSON.stringify([y.obs, y.arrows, y.labels, y.scales, my])) { diff++; if (first === null) first = i; } } return { n, diffFrames: diff, first, obsDiff, arrDiff, metaKeysDiffer: [...metaKeys] }; };
...
if (existsSync(join(EXTRA, "index.json"))) for (const n of loadRuns(EXTRA)) { const r = loadRun(EXTRA, n); results.push(auditRun(r, "extra"));
  if (n.startsWith("official-")) { const name = n.replace("official-", "").replace(/-long$/, ""); const d = loadRun(DATA, name); consistency.push({ name, vs: "r5 re-export (full, incl. meta.tf)", ...cmpRuns(d, r, true) }); }
  if (existsSync(join(R4, n + ".json"))) { const r4 = loadRun(R4, n); const c = cmpRuns(r4, r, false); consistency.push({ name: n, vs: "r4 extra (model+arrows+labels+scales+meta except tf)", ...c, tfR4: r4.frames[0].meta.tf, tfR5: r.frames[0].meta.tf }); } }
for (const n of loadRuns(DATA)) if (existsSync(join(R4, "official-" + n + ".json"))) { const r4 = loadRun(R4, "official-" + n); const c = cmpRuns(r4, loadRun(DATA, n), false); consistency.push({ name: n, vs: "data/ vs r4 official re-export (meta except tf)", ...c, tfR4: r4.frames[0].meta.tf, tfR5: loadRun(DATA, n).frames[0].meta.tf }); }
writeFileSync(join(HERE, "results-r5.json"), JSON.stringify({ results, consistency }, null, 1));
console.log("## 一致性（r5 data vs r5 重匯出；r5 vs r4 model 輸出）"); for (const c of consistency) console.log(JSON.stringify(c));
for (const r of results) {
  ...
  console.log("  tf analytic A=" + r.expected.tfA + " B=" + r.expected.tfB + " RK4=" + r.expected.tfRK4 + " | data tf=" + r.data.tfLast + " t=" + r.data.tLast + " | meta.tf=" + r.data.metaTf + " rule=" + r.expected.metaTf + " margin=" + r.data.tfMargin + " marginOk=" + r.data.tfMarginOk + " windowCoversLanding=" + r.data.windowCoversLanding + " | doneFirst=" + r.data.doneFirst + " doneAt=" + r.data.doneAt + " tAfterDone=" + r.data.tAfterDoneChanges + " doneBeforeLanded=" + r.data.doneBeforeLanded);
