// 第 5 輪（模型 0.2.3；model.ts 未改、plan 只在情景 1 meta 補 mA、mB）核數員的獨立計算：與 independent4.mjs 相同，只改輸出檔名。不讀 model.ts / plan.ts / Scene.tsx。
...
function clothStep(st, p, t, dt) {
  ...
  for (let guard = 0; rem > 1e-15 && guard < 20; guard++) {
    ...
    if (ev === "catch" && !st.left) { st.stuck = true; st.tStuck = t; }
    rem -= te;
  }
  return st;
}
...
function compareRun(run, opt = {}) {
  ...
  if (scene === "space") { st = { s: 0, v: 0 }; stB = { s: 0, v: 2 }; stC = { s: 0, v: -2 }; }
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    if (scene === "table") {
      ...
      const dtPull = st.left ? st.leftAt : (st.stuck ? st.tStuck : t);
      chk("dtPull", dtPull, o.dtPull);
      ...
      chk("s", st.s, o.s); chk("v", st.v, o.v); chk("a", a, o.a); chk("Fe", Fe, o.Fe); chk("Fnet", Fe, o.Fnet);
      ...
      if (p.trio) { chk("sB", stB.s, o.sB); chk("vB", stB.v, o.vB); chk("sC", stC.s, o.sC); chk("vC", stC.v, o.vC); }
      for (const q of [st, stB, stC]) { q.s += q.v * dt + 0.5 * a * dt * dt; q.v += a * dt; }
    }
  }
  return errs;
}
...
const fromChange = c => c ? ((t, b) => (t >= c.t - 1e-12 ? { ...b, ...c.params } : b)) : undefined;
for (const r of [...idx.runs, ...idxX.runs]) {
  const run = load(DATA + r.name + ".json");
  const opt = { sched: fromChange(run.change) };
  out.frameCompare[r.name] = compareRun(run, opt);
  out.arrows[r.name] = arrowCheck(run, opt);
...
// B. 補跑運行
const extraNames = fs.readdirSync(EXTRA).filter(f => f.endsWith(".json")).map(f => f.replace(".json", "")).filter(n => !idx.runs.some(r => r.name === n) && !idxX.runs.some(r => r.name === n));
...
fs.writeFileSync(ROOT + "audit/independent-results-r6.json", JSON.stringify(out, null, 1));
