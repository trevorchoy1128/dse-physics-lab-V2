// 第 8 輪獨立核數（v0.4 規格；第 7 輪 F1 修正後、乘客 f 預設 150 N）。不讀 model.ts / plan.ts / Scene.tsx。沿 r7-independent.mjs，改讀 audit/r8-data，並補 F1、停止瞬間、不放手、巴士 150 N 的專項檢查。
// 按規格 §7 方程以解析解（分段常加速度、事件驅動：越過 B、停止、追上布速、布邊離開、巴士相位、乘客與車同速）逐幀重算，
// 與 data/ 25 個運行、audit/r8-data/ 104 個運行的 observe 與 plan 箭嘴比對；並逐條計算 §10 條件 1–17。
// 用法：node reports/inertia-and-newtons-first-law/audit/r8-independent.mjs
...
const runs = [...loadDir(path.join(ROOT, "data")).map(r => ({ ...r, set: "data" })), ...loadDir(path.join(ROOT, "audit/r8-data")).map(r => ({ ...r, set: "r8" }))];
...
function tableAdvance(st, lane, Fapp, dt) {
  ...
  if (st.v !== 0 && Math.abs(st.v) < 1e-12) { st.v = 0; snapCount.table++; }
}
export const snapCount = { table: 0, cloth: 0 };
...
function clothAdvance(st, p, dt) {
  ...
  if (st.phase === 2 && st.v !== 0 && Math.abs(st.v) < 1e-12) { st.v = 0; st.phase = 3; snapCount.cloth++; }
}
...
say(`我的解析模型 |v| < 1e-12 歸零次數：桌面 ${snapCount.table}、桌布 ${snapCount.cloth}`);
...
{ // 7
  cond[7] = {};
  for (const n of ["extra-release-1s", "release-1.5-f2-0.4", "release-1.5-f2-0.4-dt001", "release-1.5-f2-0.05", "release-1.5-f2-0.05-dt001", "release-1.5-f2-0.25", "release-1.0-f2-0.4", "release-1.5-f2-3"]) {
...
{ // 13
  const b = R("bus-brake-f150"); const m = 60; const lim = b.params.fBus / m; const sl = b.frames.filter(f => f.obs.sliding === 1 && f.t < b.frames.at(-1).t);
  ...
  cond[13] = {
    ...
    f0: { v_bus_f0_run: uniq(R("bus-f0").frames.map(f => f.obs.v)), sRel_end_bus_f0_run: R("bus-f0").frames.at(-1).obs.sRel, v: uniq(R("extra-bus-f0").frames.map(f => f.obs.v)), sRel_end: R("extra-bus-f0").frames.at(-1).obs.sRel, s_end: R("extra-bus-f0").frames.at(-1).obs.s },
...
{ // 17
  ...
  const bb = R("bus-brake-f150").frames.find(f => f.obs.busPhase === 2 && f.obs.sliding === 1); const bf = bb.arrows.find(a => a.kind === "friction");
...
// ---------- 第 8 輪專項 ----------
{ // F1：f布 = 0 時物件不得自己動；停止瞬間／v = 0 任何幀不得報滑動摩擦
  const f1 = {}; for (const n of ["cloth-f-0", "cloth-f0-fT2-dt001", "cloth-f0-fT0.05", "cloth-f0-fT3-v0.1-L1", "cloth-f0-v10-L0.1", "edge-cloth-f0-fT0", "cloth-f0.05-fT3"]) { const r = R(n); const fr = r.frames; f1[n] = { fCloth: r.params.fCloth, fTable: r.params.fTable, tEnd: fr.at(-1).t, minS: Math.min(...fr.map(f => f.obs.s)), maxAbsS: Math.max(...fr.map(f => Math.abs(f.obs.s))), minSlide: Math.min(...fr.map(f => f.obs.slide)), maxAbsV: Math.max(...fr.map(f => Math.abs(f.obs.v))), phase_end: fr.at(-1).obs.phase, frames_v0_f_nonzero_after_leave: fr.filter(f => f.obs.phase >= 2 && f.obs.v === 0 && f.obs.f !== 0).length, frames_v0_a_nonzero: fr.filter(f => f.obs.v === 0 && f.obs.a !== 0).length, friction_arrows_after_leave: uniq(fr.filter(f => f.obs.phase >= 2).map(f => f.arrows.filter(a => a.kind === "friction").length)), dv_end: fr.at(-1).obs.dv, J_end: fr.at(-1).obs.J, worst: results[n].worst }; }
  cond.F1 = f1;
}
{ // 停止瞬間（桌面：放手後 v = 0 的第一幀；不放手：F < f₂ 停下）與停後
  const rows = {}; for (const n of ["release-1.5-f2-0.4", "release-1.5-f2-0.4-dt001", "release-1.5-f2-0.05", "release-1.5-f2-0.05-dt001", "release-1.5-f2-0.25", "release-1.0-f2-0.4", "extra-release-1s", "hold-f2-0.4", "hold-f2-0.4-dt001", "hold-f2-0.35", "hold-f2-3", "extra-table-hold-6s"]) { const r = R(n); const fr = r.frames; const iStop = fr.findIndex((f, i) => i > 0 && fr[i - 1].obs.v > 0 && f.obs.v === 0); const st = iStop >= 0 ? fr[iStop] : null; const after = iStop >= 0 ? fr.slice(iStop) : []; const F = r.change ? 0 : r.params.F; rows[n] = { dt: r.dt, F_after: F, f2: r.params.f2, t_stop: st?.t, v_prev: iStop > 0 ? fr[iStop - 1].obs.v : null, stopFrame: st ? { f: st.obs.f, a: st.obs.a, Fnet: st.obs.Fnet, nForces: st.obs.nForces, nHoriz: st.obs.nHoriz, frictionArrows: st.arrows.filter(a => a.kind === "friction").length, accelArrows: st.arrows.filter(a => a.kind === "acceleration").length } : null, after: { n: after.length, f: uniq(after.map(f => f.obs.f)), a: uniq(after.map(f => f.obs.a)), Fnet: uniq(after.map(f => f.obs.Fnet)), v: uniq(after.map(f => f.obs.v)), s: uniq(after.map(f => f.obs.s)), nForces: uniq(after.map(f => f.obs.nForces)), frictionArrows: uniq(after.map(f => f.arrows.filter(a => a.kind === "friction").length)) }, f_expected_after_stop: Math.abs(F) <= r.params.f2 ? -F : null, s_stop_exp: n.startsWith("hold") || n === "extra-table-hold-6s" ? 0.75 + 0.2 * 1.5 * 1.5 / (2 * (r.params.f2 - 0.3)) : null }; }
  cond.stopInstant = rows;
}
{ // 巴士 150 N：解析 s_rel（起步 −(1/2)(f/m − a)t₁² − (v − f/m·t₁)²/(2 f/m)；煞車對稱 → 終值 0）
  const ana = (a, V, f, m) => { const k = f / m; if (k >= a) return { sRelMin: 0, sRelEnd: 0, tCatch: null }; const t1 = V / a; const v1 = k * t1; const s1 = 0.5 * (k - a) * t1 * t1; const tc = (V - v1) / k; const s2 = -(V - v1) * tc / 2; return { sRelMin: s1 + s2, tCatchStart: t1 + tc, sRelEnd: 0, tStopPax: 2 * t1 + 4 + tc }; };
  const rows = {}; for (const [n, f] of [["bus-brake-f150", 150], ["bus-brake-f150-dt001", 150], ["scenario-bus-brake", 150], ["bus-brake-f120", 120], ["bus-f170", 170], ["bus-f180-boundary", 180], ["bus-start-f240", 240], ["bus-f0", 0], ["extra-bus-f0", 0], ["scenario-bus-start", 0]]) { const r = R(n); const fr = r.frames; const e = ana(r.params.aBus, r.params.vBus, r.params.fBus, 60); const stopSlide = []; for (let i = 1; i < fr.length; i++) if (fr[i - 1].obs.sliding === 1 && fr[i].obs.sliding === 0) stopSlide.push(fr[i].t); const start = fr.filter(x => x.obs.busPhase === 0), brake = fr.filter(x => x.obs.busPhase === 2); rows[n] = { f, dt: r.dt, tEnd: fr.at(-1).t, lim: f / 60, sRel_min: Math.min(...fr.map(x => x.obs.sRel)), sRel_min_exp: e.sRelMin, sRel_max: Math.max(...fr.map(x => x.obs.sRel)), sRel_end: fr.at(-1).obs.sRel, sRel_end_exp: e.sRelEnd, tCatchStart_exp: e.tCatchStart, tStopPax_exp: e.tStopPax, stopSlideTimes: stopSlide, a_start: uniq(start.map(x => x.obs.a)), a_brake: uniq(brake.map(x => x.obs.a)), f_start: uniq(start.map(x => x.obs.f)), f_brake: uniq(brake.map(x => x.obs.f)), backwardArrows_start: start.filter(x => x.arrows.some(a => (a.kind === "friction" || a.kind === "tension") && a.vector[0] < 0)).length, forwardArrows_brake: brake.filter(x => x.arrows.some(a => (a.kind === "friction" || a.kind === "tension") && a.vector[0] > 0)).length, v_end: fr.at(-1).obs.v, s_end: fr.at(-1).obs.s, sBus_end: fr.at(-1).obs.sBus, nHoriz_max: Math.max(...fr.map(x => x.obs.nHoriz)), worst: results[n].worst }; }
  cond.bus150 = rows;
}
say("== 條件 ==");
for (const [k, v] of Object.entries(cond)) say(`[${k}] ${JSON.stringify(v)}`);
fs.writeFileSync(path.join(ROOT, "audit/r8-results.json"), JSON.stringify({ results, cond, scaleViol: scaleViol.slice(0, 20), kindViol, presenceViol: presenceViol.slice(0, 50), dirViol: dirViol.slice(0, 50) }, null, 1));
fs.writeFileSync(path.join(ROOT, "audit/r8-independent.log"), log.join("\n"));
