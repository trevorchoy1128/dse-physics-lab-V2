// 第 8 輪額外檢查（規格 v0.4）：§10 驗證條件的解析核對 + 量綱 / 對稱 / 極限。只用第二實作 model.mjs。
// 第 8 輪改動：base.fBus 120 → 150（§5、controls.ts）；巴士數值期望按 f/m = 2.5 重算；新增 §7 邊界三組檢查（見「第 8 輪新增」）。
...
const base = {
  ...
  vCloth: 5, fCloth: 1.5, fTable: 2, L: 0.4, mObj: 1, aBus: 3, vBus: 10, fBus: 150, handrail: false,
...
{ // 13 巴士
  ...
  const pS = { ...base, scene: "bus" }; // f/m = 2.5 < 3
  const frS = sim(pS, 17, 0.005);
  const brake = frS.filter(f => f.obs.busPhase === 2);
  check("§10-13 f/m < a：滑動時乘客 |a| = f/m；煞車期間 v乘 ≥ v車", frS.filter(f => f.obs.sliding).every(f => rel(Math.abs(f.obs.a), 2.5) < 1e-12) && brake.every(f => f.obs.v >= f.obs.vBus - 1e-12));
  const stopSlide = frS.find(f => f.obs.busPhase === 3 && f.obs.sliding === 0);
  // 煞車期間乘客亦以 f/m = 2.5 減速：t₃ 時 v乘 = 10 − 2.5·(10/3) = 1.667，再滑 1.667/2.5 = 0.667 s → 停於 t₃ + 0.667 = 11.333 s
  const tStopExp = 10 / 3 * 2 + CRUISE + (10 - 2.5 * (10 / 3)) / 2.5;
  check("§10-13 相對滑動在 v乘 = v車 時停止（巴士停定後乘客滑至停，t = 11.333 s）", stopSlide && stopSlide.obs.v === 0 && Math.abs(stopSlide.t - tStopExp) <= 0.005 + 1e-9, `t = ${stopSlide?.t} vs ${tStopExp}`);
  ...
  check("§10-13 握扶手：sRel ≡ 0、f + Fhand = m a車、|f| ≤ f 設定", frH.every(f => f.obs.sRel === 0 && rel(f.obs.f + f.obs.Fhand, M_PASS * f.obs.aBusNow) < 1e-12 || (f.obs.aBusNow === 0 && f.obs.Fnet === 0)) && frH.every(f => Math.abs(f.obs.f) <= 150 + 1e-12));
  check("§10-13 握扶手預設：起步 f = 150 N、Fhand = 30 N、Fnet = 180 N", frH[0].obs.f === 150 && frH[0].obs.Fhand === 30 && frH[0].obs.Fnet === 180);
  // §5 v0.4：預設 f = 150 N 起步相對向後滑 3.33 m、煞車相對向前滑 3.33 m；解析 |sRel|max = v²(a − f/m)/(2 a f/m) = 100·0.5/(2·3·2.5) = 3.333
  //（起步段 sRel 由 0 降到 −3.33 後保持；煞車段由 −3.33 回升 +3.33 到 0：兩段相對位移大小相等、方向相反，終值 0——起步／煞車鏡像）
  const sRelExp = 100 * (3 - 2.5) / (2 * 3 * 2.5);
  const atT2 = frS.filter(f => f.obs.busPhase === 1).at(-1).obs.sRel /* 巡航相最後一幀（追上後 sRel 恆定） */, sRelMin = Math.min(...frS.map(f => f.obs.sRel)), sRelEnd = last(frS).sRel;
  check("§5 預設 f = 150 N：起步相對向後滑 3.33 m、煞車相對向前滑 3.33 m（解析 v²(a − f/m)/(2a·f/m)），終值 sRel = 0", rel(-sRelMin, sRelExp) < 1e-9 && rel(atT2, -sRelExp) < 1e-9 && rel(sRelEnd - atT2, sRelExp) < 1e-9 && Math.abs(sRelEnd) < 1e-9, `起步 ${sRelMin.toFixed(4)}，煞車 +${(sRelEnd - atT2).toFixed(4)}，終值 ${sRelEnd.toExponential(2)}`);
  // 起步段乘客追上巴士：t₁ 時 v乘 = 2.5·3.333 = 8.333，巡航期間再 (10 − 8.333)/2.5 = 0.667 s → t = 4.0 s（< t₂ = 7.333）
  const catchUp = frS.find(f => f.obs.busPhase === 1 && f.obs.sliding === 0);
  check("§10-13 預設 f = 150 N：起步後於巡航期 t = 4.0 s 追上巴士（相對滑動停止）", catchUp && Math.abs(catchUp.t - 4) <= 0.005 + 1e-9, `t = ${catchUp?.t}`);
}
{ // 15 能量
  const p = { ...base };
  const fr = sim(p, 6, 0.001, { t: 1, params: { push: "off" } });
...
// ---------- 對稱 ----------
{
  // 預設 f = 150（f/m = 2.5 < 3）：起步段乘客 a = +2.5 追不上；巡航期間 t = 4 s 追上（< t₂ = 7.33）；煞車段全程 a = −2.5
  const fr = sim({ ...base, scene: "bus", fBus: 150 }, 17, 0.005);
  const t1 = 10 / 3;
  const start = fr.filter(f => f.obs.busPhase === 0), brake = fr.filter(f => f.obs.busPhase === 2);
  // 起步：乘客 a = +f/m，摩擦 +f；煞車：a = −f/m，摩擦 −f（鏡像）
  check("對稱：起步／煞車鏡像（a = ±f/m、f = ±f 設定）", start.every(f => f.obs.a === 2.5 && f.obs.f === 150) && brake.every(f => f.obs.a === -2.5 && f.obs.f === -150));
...
// ---------- 第 8 輪新增：§7 邊界 ----------
{ // (i) 預設參數不放手（extra-table-hold-6s 的解析解）：F = 0.30 < f₂ = 0.40，過 B 後 a = −0.5，t = 4.0 s 停於 s = 3.0 m，之後靜摩擦 −0.30、淨力 0
  const p = { ...base };
  const fr = sim(p, 12, 0.005);
  const stopFr = fr.find(f => f.t > 1 && f.obs.v === 0);
  check("§7 不放手：t = 4.0 s 停於 s = 3.0 m（解析 t_B + m v_B/(f₂ − F)、s = L_AB + m v_B²/(2(f₂ − F))）", stopFr && Math.abs(stopFr.t - 4) <= 0.005 + 1e-9 && rel(stopFr.obs.s, 3) < 1e-9, `t = ${stopFr?.t}, s = ${stopFr?.obs.s}`);
  const after = fr.filter(f => f.t >= 4 - 1e-9);
  check("§7 不放手：停下後摩擦 = −F = −0.30（不是 −f₂）、Fnet = 0、a = 0、v ≡ 0、nHoriz = 2、nForces = 4", after.every(f => f.obs.f === -0.3 && f.obs.Fnet === 0 && f.obs.a === 0 && f.obs.v === 0 && f.obs.nHoriz === 2 && f.obs.nForces === 4), `停下後幀數 ${after.length}`);
  const before = fr.filter(f => f.t > 1 + 1e-9 && f.t < 4 - 1e-9);
  check("§7 不放手：B 至停下期間摩擦 = −f₂ = −0.40、Fnet = −0.10、a = −0.50", before.every(f => f.obs.f === -0.4 && rel(f.obs.Fnet, -0.1) < 1e-9 && rel(f.obs.a, -0.5) < 1e-9));
  // 停止事件恰落步末：dt 0.005 與 0.001 下 t = 4.000 是幀；dt 0.003／0.007 停下落在步中，下一幀 v 亦須為 0 而非負
  for (const dt of [0.001, 0.003, 0.007]) {
    const fr2 = sim(p, 6, dt);
    const s0 = fr2.find(f => f.t > 1 && f.obs.v === 0);
    check(`§7 不放手 dt = ${dt}：停下幀 t ∈ [4.0, 4.0 + dt]、v 永不為負、停下幀起 f = −0.30`, s0 && s0.t >= 4 - 1e-9 && s0.t <= 4 + dt + 1e-9 && fr2.every(f => f.obs.v >= 0) && fr2.filter(f => f.t >= s0.t).every(f => f.obs.f === -0.3), `t = ${s0?.t}`);
  }
}
{ // (ii) f布 = 0：物件 s ≡ 0、slide ≡ 0、離開一瞬 v = 0 → 直接靜止，任何幀摩擦都不是 −f桌
  for (const [vc, dt] of [[5, 0.001], [5, 0.005], [1.3, 0.001], [0.2, 0.005]]) {   // v布 = 0.1 時 t抽 = L/v布 = 4 s > 時間窗 T = 3 s，畫面凍結前抽不完（見待釐清）
    const fr = sim({ ...base, scene: "cloth", fCloth: 0, vCloth: vc }, 3, dt);
    const ok = fr.every(f => f.obs.s === 0 && f.obs.slide === 0 && f.obs.v === 0 && f.obs.f >= 0 && f.obs.phase !== 2 && f.obs.dv === 0 && f.obs.J === 0);
    const leave = fr.find(f => f.obs.phase === 3);
    check(`§7 f布 = 0（v布 = ${vc}, dt = ${dt}）：s ≡ 0、slide ≡ 0、v ≡ 0、無 phase 2、無 −f桌；t抽 = L/v布 = ${(0.4 / vc).toFixed(3)} s`, ok && leave && Math.abs(leave.t - 0.4 / vc) <= dt + 1e-9, `phase 3 起於 t = ${leave?.t}`);
  }
  const fr0 = sim({ ...base, scene: "cloth", fCloth: 0, fTable: 0 }, 3, 0.001);
  check("§7 f布 = f桌 = 0：s ≡ 0、phase 0 → 3、f ≡ 0（不會因 f桌 = 0 卡在滑動相）", fr0.every(f => f.obs.s === 0 && f.obs.f === 0 && f.obs.phase !== 2));
}
{ // (iii) v_rel = 0 的幀摩擦不得為滑動值：|f| ≤ |F_需|，a = 0 ⇔ f = −F_需（掃描多組參數與步長，含步末命中）
  let n = 0, badN = 0, first = null;
  const tableCases = [
    [{ ...base }, 12, 0.005], [{ ...base }, 6, 0.003], [{ ...base, f1: 0.4, F: 0.4 }, 2, 0.01], [{ ...base, f1: 0.4, F: 0.5 }, 2, 0.01],
    [{ ...base, F: 0, f1: 0.4 }, 2, 0.01], [{ ...base, f1: 0.2, f2: 0.4 }, 12, 0.005], [{ ...base, f2: 3 }, 12, 0.005],
  ];
  for (const [p, T, dt] of tableCases) for (const f of sim(p, T, dt, null)) {
    if (f.obs.v !== 0) continue; n++;
    const o = f.obs, ok = Math.abs(o.f) <= o.Fapp + 1e-12 && (o.a === 0 ? o.f === -o.Fapp : o.Fapp > Math.abs(o.f) - 1e-12);
    if (!ok) { badN++; first = first ?? { scene: 1, t: f.t, o }; }
  }
  for (const f of sim({ ...base }, 6, 0.001, { t: 1, params: { push: "off" } })) {
    if (f.obs.v !== 0 || f.t < 1) continue; n++;   // 放手後靜止：F_需 = 0 → f = 0
    if (!(f.obs.f === 0 && f.obs.a === 0)) { badN++; first = first ?? { scene: 1, t: f.t, o: f.obs }; }
  }
  for (const vc of [5, 1.2, 1.0, 0.5]) for (const f of sim({ ...base, scene: "cloth", vCloth: vc }, 8, 0.001)) {
    if (f.obs.v !== 0) continue; n++;
    if (!(f.obs.f >= 0 && (f.obs.phase < 2 || (f.obs.f === 0 && f.obs.a === 0)))) { badN++; first = first ?? { scene: 2, t: f.t, o: f.obs }; }
  }
  for (const fb of [0, 100, 150, 180, 200, 300]) for (const hr of [false, true]) for (const f of sim({ ...base, scene: "bus", fBus: fb, handrail: hr }, 17, 0.005)) {
    if (f.obs.v !== f.obs.vBus) continue; n++;
    const need = M_PASS * f.obs.aBusNow, o = f.obs;
    let ok = Math.abs(o.f) <= Math.abs(need) + 1e-9 && (o.f === 0 || Math.sign(o.f) === Math.sign(need));
    if (hr) ok = ok && Math.abs(o.f + o.Fhand - need) <= 1e-9;
    if (!ok) { badN++; first = first ?? { scene: 3, t: f.t, o }; }
  }
  check(`§7 v_rel = 0 的幀：|f| ≤ |F_需|、a = 0 ⇔ f = −F_需（三個情景共 ${n} 幀）`, badN === 0, badN ? JSON.stringify(first) : "0 違規");
}
